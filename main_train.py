...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    #         if seg == '<':
    #             mark_index.append(str(id + id_2 + 1))
    #             check = 1
    #         if check == 1 and seg != '<' and seg != '>':
    #             mark_index.append(str(id + id_2 + 1))
    #         if check == 1 and seg == '>':
    #             mark_index.append(str(id + id_2 + 1))
    #             break
    ...
    # try:
    #     check = 0
    #     for id_3,seg in enumerate(tokens[id_2 + id + 2:]):
    #         if seg == '<':
    #             mark_index.append(str(id + id_2 + id_3 + 2))
    #             check = 1
    #         if check == 1 and seg != '<' and seg != '>':
    #             mark_index.append(str(id + id_2 + id_3 +  2))
    #         if check == 1 and seg == '>':
    ...
    # try:
    #     check = 0
    #     for id_4,seg in enumerate(tokens[id_3 + id_2 + id + 3:]):
    #         if seg == '<':
    #             mark_index.append(str(id + id_2 + id_3 + id_4 + 3))
    #             check = 1
    #         if check == 1 and seg != '<' and seg != '>':
    #             mark_index.append(str(id + id_2 + id_3 + id_4 + 3))
    #         if check == 1 and seg == '>':
    ...
    # try:
    #     check = 0
    #     for id_5,seg in enumerate(tokens[id_4 + id_3 + id_2 + id + 4:]):
    #         if seg == '<':
    #             mark_index.append(str(id + id_2 + id_3 + id_4 + id_5 + 4))
    #             check = 1
    #         if check == 1 and seg != '<' and seg != '>':
    #             mark_index.append(str(id + id_2 + id_3 + id_4 + id_5  + 4))
    #         if check == 1 and seg == '>':
    ...
    if feature_length < 41:
        for add_id in range(41-feature_length):
            feature_index.append(str('0'))

    mark_length = len(mark_index)
    if mark_length > 28:
        mark_index = mark_index[0:28]
    if mark_length < 28:
        for add_id in range(28-mark_length):
            mark_index.append(str('0'))
    ...
    def __init__(self, embedding, embedding_mark, embedding_feature,embedding_word, num_classes, embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0001):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.variable_scope('conv-max-pooling-%s' % filter_size):
                ...
                # conv = tf.concat([conv_1,conv_3],1)
                pooled = tf.reduce_max(conv_1, reduction_indices=[1], name='max-pooling')
                pooled_outputs.append(pooled)
